...
#### Q4. "엣지 모드에서 AI가 먹통이에요!"
*   **진단**: `assets/models` 폴더에 `.tflite` 파일이 없거나, 폰 기종이 너무 구형일 수 있습니다.
*   **해결**: `flutter run` 할 때 콘솔에 `TFLite Error`가 뜨는지 확인하고, 모델 파일이 제대로 복사되었는지(`pubspec.yaml`) 확인하세요.
